package com.sinarkelas.ui.videocallgroup.services
...
import com.sinarkelas.ui.videocallgroup.db.QbUsersDbManager
import com.sinarkelas.ui.videocallgroup.fragment.CAMERA_ENABLED
import com.sinarkelas.ui.videocallgroup.fragment.IS_CURRENT_CAMERA_FRONT
import com.sinarkelas.ui.videocallgroup.fragment.MIC_ENABLED
import com.sinarkelas.ui.videocallgroup.fragment.SPEAKER_ENABLED
import com.sinarkelas.ui.videocallgroup.videoutil.NetworkConnectionChecker
...
class CallService : Service() {
    ...
    override fun onCreate() {
        currentSession = WebRtcSessionManager.WebRtcSessionManager.getCurrentSession()
        clearButtonsState()
        initNetworkChecker()
        initRTCClient()
        initListeners()
        initAudioManager()
        ringtonePlayer = RingtonePlayer(this, com.sinarkelas.R.raw.beep)
        super.onCreate()
    }
    ...
    private fun initNotification(): Notification {
        ...
        val notificationTitle = getString(com.sinarkelas.R.string.notification_title)
        var notificationText = getString(com.sinarkelas.R.string.notification_text, "")

        val callTime = getCallTime()
        if (!TextUtils.isEmpty(callTime)) {
            notificationText = getString(com.sinarkelas.R.string.notification_text, callTime)
        }
        ...
        val channelId: String = if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            ...
            getString(com.sinarkelas.R.string.app_name)
        }
        ...
        builder.setSmallIcon(com.sinarkelas.R.mipmap.ic_launcher)
        val bitmapIcon = BitmapFactory.decodeResource(resources, com.sinarkelas.R.mipmap.ic_launcher)
    ...
    private fun createNotificationChannel(channelId: String, channelName: String): String {
        ...
        channel.lightColor = getColor(com.sinarkelas.R.color.green)
    ...
    private inner class ConnectionListenerImpl : AbstractConnectionListener() {
        override fun connectionClosedOnError(e: Exception?) {
            ...
            val reconnectHangUpTimeMillis = getPreferenceInt(sharedPref, applicationContext,
                com.sinarkelas.R.string.pref_disconnect_time_interval_key,
                com.sinarkelas.R.string.pref_disconnect_time_interval_default_value) * 1000
    ...
    private inner class SessionEventsListener : QBRTCClientSessionCallbacks {
        ...
        override fun onSessionStartClose(session: QBRTCSession?) {
            if (session == WebRtcSessionManager.WebRtcSessionManager.getCurrentSession()) {
        ...
        override fun onReceiveHangUpFromUser(session: QBRTCSession?, userID: Int?, p2: MutableMap<String, String>?) {
            ...
            shortToast("User " + participantName + " " + getString(com.sinarkelas.R.string.text_status_hang_up) + " conversation")
        }
        ...
        override fun onReceiveNewSession(session: QBRTCSession?) {
            ...
            WebRtcSessionManager.WebRtcSessionManager.getCurrentSession()?.let {
        ...
        override fun onUserNoActions(p0: QBRTCSession?, p1: Int?) {
            ...
            clearCallState()
            clearButtonsState()
            WebRtcSessionManager.WebRtcSessionManager.setCurrentSession(null)
            CallService.stop(this@CallService)
        }
    ...
    private inner class QBRTCSignalingListener : QBRTCSignalingCallback {
        ...
        override fun onErrorSendingPacket(p0: QBSignalingSpec.QBSignalCMD?, p1: Int?, p2: QBRTCSignalException?) {
            shortToast(com.sinarkelas.R.string.dlg_signal_error)
        }
    }
